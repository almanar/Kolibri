...
void swap_channel(VMUINT8 index)
{
...
VMUINT8 *dexg4_recieve(){
	...
	do {
		...
		delay(50);
		vm_log_info("slen: %d", cc2500_read_status_reg(REG_PKTLEN));
			...
			if (get_packet_passed_checksum(dexg4_packet.lqi)){
				...
				if ( strcmp( ASCII_SRC_NAME, CM_TRANSMITTER_ID) == 0 ){
					if (RX_POLL_TIME == 0 && channel == 0){
						RX_POLL_TIME = post_ch_timmer[0] - 475;
						vm_time_get_unix_time(&RX_POLL_TIME_EPOCH);
						RX_POLL_TIME_EPOCH --;
					}
